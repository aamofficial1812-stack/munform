...
(function () {
  ...
  /* --------------------------------------------------------
     Submission
     Keeps all collected data in one clean object, ready to be
     sent to a Google Apps Script endpoint (or any backend) later.
     The payment screenshot File object is included for a future
     multipart/form-data or base64 handoff — it is not uploaded
     anywhere by this frontend-only build.
     -------------------------------------------------------- */
  function submitRegistration() {
    const registrationData = {
      committee: state.committee,
      registrationType: state.registrationType,
      numberOfDelegates: state.numberOfDelegates,
      ...
      paymentScreenshot: state.paymentScreenshot
    };

    // Placeholder for future integration, e.g.:
    // const payload = new FormData();
    // Object.entries(registrationData).forEach(([key, value]) => payload.append(key, value));
    // fetch("YOUR_GOOGLE_APPS_SCRIPT_URL", { method: "POST", body: payload });

    console.log("AIKTCMUN 2026 registration:", registrationData);

    form.hidden = true;
    document.querySelector(".progress-nav").hidden = true;
