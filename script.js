...
(function () {
  ...
  function validateStep(stepNumber) {
    ...
    if (stepNumber === 4) {
      ...
      if (!count || count < 1) {
        ...
        setError("numberOfDelegates", "Maximum 6 delegates allowed per registration.");
        valid = false;
      } else {
  ...
  function updatePricing() {
    ...
    stepperChips.forEach((chip) => {
      ...
      chip.classList.toggle("is-active", count === chipCount);
    });
  ...
  async function submitRegistration() {
    ...
    try {
      ...
      let safeDelegateCount = parseInt(state.numberOfDelegates, 10);
      if (!safeDelegateCount || safeDelegateCount < 1) safeDelegateCount = 1;
      if (safeDelegateCount > 6) safeDelegateCount = 6;
      state.numberOfDelegates = safeDelegateCount;
