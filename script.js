...
(function () {
  ...
  /* ============================================================
     0b. REGISTRATION STATUS CONFIGURATION
     ------------------------------------------------------------
     Set REGISTRATION_CLOSED to true to show the "Registration
     Closed" screen instead of the registration wizard (used when
     committee/room capacity has been filled).

     Set it back to false to instantly restore the original
     multi-step registration form — nothing else needs to change.
     ============================================================ */

  const REGISTRATION_CLOSED = true;

  // Conference start used by the closed-screen countdown:
  // 18 September 2026, 9:00 AM India Standard Time (UTC+05:30).
  const CONFERENCE_START_ISO = "2026-09-18T09:00:00+05:30";
  ...
  /* ============================================================
     1B. ALLOTTED PORTFOLIOS (NEW — FRONTEND-ONLY CONFIGURATION)
     ------------------------------------------------------------
     These are the portfolios already assigned by the Organising
     Committee (kept as a static, local data structure per the
     requirement — no Sheet/Admin-Notes/API dependency of any kind).

     Names below are copied verbatim from the allocation sheets
     provided ("Portfolio alloted" / "Allocation" columns). Nothing
     here is invented — see ALIAS map just below for the handful of
     shorthand names that needed to be linked to their exact
     portfolio-list entry (e.g. "Germany" -> "Federal Republic of
     Germany"), and the NOTES for entries that could not be matched
     to any existing portfolio in PORTFOLIOS.
     ============================================================ */
  const ALLOTTED_PORTFOLIOS = {
    UNHRC: [
      "France",
      "Japan",
      "Brazil",
  ...
  // A handful of allocation-sheet entries use a short/common name instead
  // of the exact portfolio-list wording (e.g. "Germany" vs. "Federal
  // Republic of Germany", "Mamta Banerjee" vs. "Mamata Banerjee"). These
  // are well-known 1:1 identifications, not guesses about *whether*
  // something is allotted — only about *which existing card* a shorthand
  // name refers to. Each maps a normalized alias to the exact, unmodified
  // name already present in PORTFOLIOS.
  const ALLOTTED_ALIASES = {
    UNHRC: {
    ...
    AIPPM: {
      ...
      "mehbooba mistri": "Mehbooba Mufti",
      "priyanka gandhi": "Priyanka Gandhi Vadra"
    }
  };

  // Normalizes for safe comparison only (case + stray whitespace).
  // Never used to alter what is displayed to delegates.
  function normalizePortfolioName(str) {
    return String(str || "")
      .toLowerCase()
      .replace(/\u2019/g, "'")
      .replace(/\s+/g, " ")
  ...
  // Builds { UNHRC: Set(portfolioId), AIPPM: Set(portfolioId) } once.
  // Wrapped in try/catch per the fail-safe requirement: if this ever
  // throws, registration must keep working with nothing locked.
  function buildAllottedIndex() {
    const index = { UNHRC: new Set(), AIPPM: new Set() };
    try {
      Object.keys(ALLOTTED_PORTFOLIOS).forEach(function (committee) {
        const list = PORTFOLIOS[committee] || [];
        const byNormalizedName = {};
        list.forEach(function (item) {
          byNormalizedName[normalizePortfolioName(item.name)] = item;
        });
        const aliases = ALLOTTED_ALIASES[committee] || {};

        (ALLOTTED_PORTFOLIOS[committee] || []).forEach(function (rawName) {
          const n = normalizePortfolioName(rawName);
          let match = byNormalizedName[n];
          if (!match && aliases[n]) {
            match = byNormalizedName[normalizePortfolioName(aliases[n])];
          }
          if (match) {
            index[committee].add(match.id);
          } else if (window.console && console.warn) {
            console.warn(
              "[AIKTCMUN] Allotted portfolio \"" + rawName + "\" (" + committee +
              ") does not match any portfolio currently in the list \u2014 " +
              "it will not be locked. Add it to PORTFOLIOS or ALLOTTED_ALIASES if it should be."
            );
          }
        });
      });
    } catch (e) {
      // Fail-safe: never let a config problem block registration.
      return { UNHRC: new Set(), AIPPM: new Set() };
    }
    return index;
  }

  const ALLOTTED_INDEX = buildAllottedIndex();

  function isPortfolioAllotted(committee, portfolioId) {
    try {
      return !!(ALLOTTED_INDEX[committee] && ALLOTTED_INDEX[committee].has(portfolioId));
    } catch (e) {
  ...
  function renderPortfolioStep() {
    ...
    registrationData.delegates.forEach(function (delegate) {
      ...
      // Fail-safe consistency check: if a previously-selected preference
      // is (or has become) an allotted portfolio, clear just that
      // preference so an unavailable choice can never be submitted.
      if (delegate.committee && delegate.preference1Id && isPortfolioAllotted(delegate.committee, delegate.preference1Id)) {
        delegate.preference1 = ""; delegate.preference1Id = "";
      }
      if (delegate.committee && delegate.preference2Id && isPortfolioAllotted(delegate.committee, delegate.preference2Id)) {
  ...
  function renderPortfolioGrid(grid, delegate, query, prefNum) {
    ...
    filtered.forEach(function (item) {
      const isAllotted = isPortfolioAllotted(delegate.committee, item.id);
      const isLocked = otherPrefId === item.id;
      const isSelected = !isAllotted && ownId === item.id;
      const isUnavailable = isAllotted || isLocked;
      const card = document.createElement("button");
      card.type = "button";
      card.className = "portfolio-card" +
        (isSelected ? " is-selected" : "") +
        (isLocked ? " is-locked" : "") +
        (isAllotted ? " is-allotted" : "");
      card.disabled = isUnavailable;
      if (isAllotted) card.setAttribute("aria-disabled", "true");
      card.setAttribute("aria-pressed", String(isSelected));

      const statusText = isAllotted ? "Allotted" : (isLocked ? "Already your other preference" : "Available");
  ...
  function validateStep4() {
    ...
    registrationData.delegates.forEach(function (delegate) {
      if (!delegate.committee || !delegate.preference1Id || !delegate.preference2Id) {
        ...
        isPortfolioAllotted(delegate.committee, delegate.preference1Id) ||
        isPortfolioAllotted(delegate.committee, delegate.preference2Id)
      ) {
        // Defense in depth: should be unreachable via the UI, but never
        // let an allotted portfolio pass through to submission.
        setError("portfolio-" + delegate.id, "One of your selected portfolios has just been allotted. Please choose a different portfolio.");
        ok = false;
      }
    });
    return ok;
  ...
  function startClosedCountdown() {
    const targetTime = new Date(CONFERENCE_START_ISO).getTime();
    ...
    function tick() {
      ...
      if (remaining <= 0) {
        ...
        if (labelEl) labelEl.textContent = "AIKTCMUN 2026 Is Now Underway";
